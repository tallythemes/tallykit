<?php
class tallykit_FrontPage_block_output_logo_grid{
	public $prefix;
	...
	function content(){
		...
		if($enable == 'on'):
			echo '<div class="front_page_logo_grid">';
				...
				$output = '[tk_logo_grid category="'.$category.'" exclude_category="" relation="'.$relation.'" limit="'.$limit.'" columns="'.$columns.'" orderby="'.$orderby.'" order="'.$order.'" ids="" filter="'.$filter.'" margin="'.$margin.'" /]';
