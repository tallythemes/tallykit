	...
	function content(){
		...
		if($enable == 'on'):
			echo '<div class="front_page_logo_carousel">';
				...
				$output = '[tk_logo_carousel category="'.$category.'" exclude_category="" relation="'.$relation.'" limit="'.$limit.'" orderby="'.$orderby.'" order="'.$order.'" ids="" control_nav="'.$control_nav.'" direction_nav="'.$direction_nav.'" item_width="'.$item_width.'" item_margin="'.$item_margin.'" min_items="'.$min_items.'" max_items="'.$max_items.'" move="'.$move.'" /]';
