	...
	function content(){
		...
		if($enable == 'on'):
			echo '<div class="front_page_testimonial_grid">';
				...
				$output = '[tk_testimonial_grid category="'.$category.'" exclude_category="" relation="'.$relation.'" limit="'.$limit.'" columns="'.$columns.'" orderby="'.$orderby.'" order="'.$order.'" ids="" filter="'.$filter.'" margin="'.$margin.'" /]';
